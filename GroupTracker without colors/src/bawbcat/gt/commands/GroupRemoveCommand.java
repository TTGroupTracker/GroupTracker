    ...
    public boolean action(String message, String sender, String channel, GroupTracker bot) {
        ...
        if (removedGroup != null) {
            ...
            if (groupOwner.equals(sender)) {
                ...
                bot.sendNotice(sender, Colors.NORMAL + groupOwner + "'s " + Colors.NORMAL + removedGroup.getTypeString() + " has been terminated.");
                bot.sendNotice(groupOwner, "Your " + removedGroup.getTypeString() + " has been forcefully terminated by " + Colors.NORMAL + sender + Colors.NORMAL + ".");
            }
            for (String s2 : removedGroup.nicksComing) {
                if (!s2.equals(removedGroup.owner)) {
                    bot.sendNotice(s2, "The group you were in (" + Colors.NORMAL + groupOwner + "'s " + Colors.NORMAL + removedGroup.getTypeString() + ") has been disbanded.");
                }
            }
        } else {
            if (groupOwner.equals(sender)) {
                ...
                bot.sendNotice(sender, Colors.NORMAL + groupOwner + Colors.NORMAL + " doesn't seem to have a group!");
            }
        }
        return true;
    }

...
public class GroupInfoCommand extends Command {
    ...
    public void sendInfo(String sender, Group g, GroupTracker bot) {
        ...
        bot.sendNotice(sender, Colors.BROWN + Colors.BOLD + g.owner + Colors.NORMAL + " owns a " + g.getTypeString() + g.getLocationString() + (g.type != GroupType.BEAN_FEST ? " that has " + Colors.BOLD + g.players + "/" + g.maxPlayers + Colors.NORMAL + " players" : "") + ". Want to join them? " + (g.type != GroupType.BEAN_FEST ? "Say '" + Colors.BOLD + "join group " + g.owner + Colors.NORMAL  + "." : "Then head on over there!"));
        if (g.players > 1) {
